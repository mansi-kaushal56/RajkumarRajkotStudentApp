...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UserDefaults.standard.value(forKey: "userdata") != nil {
            ...
            let story = UIStoryboard(name: AppStoryboards.main.getDescription, bundle:nil)
            //let vc = story.instantiateViewController(withIdentifier: "splashVC")
            let vc = story.instantiateViewController(withIdentifier: AppViewControllerID.splashVC.getIdentifier )
            UIApplication.shared.windows.first?.rootViewController = vc
            UIApplication.shared.windows.first?.makeKeyAndVisible()
        }
    }
